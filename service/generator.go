...
func (s Generator) run(ctx context.Context, msg *nats.Msg) error {
	...
	zerolog.Ctx(ctx).Info().Msg("generation request received")
	defer zerolog.Ctx(ctx).Info().Msg("generation request processed")
